...
cron.schedule("0 0 0 1 * *", async () => {
  ...
  const devicesID = await Admins.find({
    $and: [{ paymentMethod_type: "اقتطاع شهري" }, { isActivePayment: true }]
  }).select("fcmToken");
  ...
  CreateNotificationMultiple(
    arr,
    "تذكير بالتبرع",
    "تنبيه بايداع دفعة التبرع وشكرا لتعاونكم معنا"
  );
...
exports.addBankDetails = async (req, reply) => {
  try {
    ...
    if (_PrevBankDetails) {
      ...
      const _bankDetails = await Admins.findByIdAndUpdate(
      ...
      CreateNotification(
        userData.fcmToken,
        "تم التحقق من المعلومات البنكية بنجاح .. يمكنكم الان التسجيل في التطبيق",
        "",
        "",
        userData._id
      );
...
exports.deActivate = async (req, reply) => {
  try {
    ...
    const _payments = await payments.findByIdAndUpdate(
      id,
      {
        isActive: false
      },
    ...
    const _admin = await Admins.findByIdAndUpdate(
      id,
      {
...
exports.addRequest = async (req, reply) => {
  try {
    ...
    let prevRequest = await requests
...
exports.updateRequestByAdmin = async (req, reply) => {
  try {
    ...
    const approveCount = await request_approve
      .find({ $and: [{ request_id: req.body.id }] })
      .count();
  ...
  try {
    ...
    await requests.findById(req.params.id).exec(async function(err, _result) {
      ...
      await payments
        .find({
          $and: [
            { to_user: _result.user_id },
            { methodFor: _result.type },
            { isActive: true },
            { status: 2 }
          ]
        })
...
exports.PaymentPerYear = async (req, reply) => {
  try {
    ...
    var items = [];
    await payments.find({ status: 2 }).exec(function(err, result) {
      result.forEach(element => {
        var month_number = new Date(element.createAt).getMonth();
        var month_name = monthNames[month_number];
        if (element.flag == 1) {
